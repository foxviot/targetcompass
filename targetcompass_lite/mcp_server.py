...
def handle_jsonrpc(project_dir: Path, message: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        if method == "resources/list":
            return _response(msg_id, {"resources": _mcp_resources(project_dir)})
        if method == "resources/read":
            uri = params.get("uri", "")
            if not uri:
                return _error(msg_id, -32602, "resources/read requires params.uri")
            result = read_resource(project_dir, uri, actor="mcp_server")
            return _response(
                msg_id,
                {
                    "contents": [
        ...
        if method == "tools/list":
            return _response(msg_id, {"tools": _mcp_tools(project_dir)})
        if method == "tools/call":
            ...
            try:
                result = call_tool(project_dir, name, arguments, actor="mcp_server")
...
def run_stdio_server(project: str, stdin: BinaryIO | None = None, stdout: BinaryIO | None = None) -> None:
    ...
    while True:
        ...
        if message is None:
            return
        response = handle_jsonrpc(project_dir, message)
...
def _mcp_resources(project_dir: Path) -> list[dict[str, Any]]:
    gateway = build_mcp_gateway(project_dir)
    resources = []
...
def _mcp_tools(project_dir: Path) -> list[dict[str, Any]]:
    gateway = build_mcp_gateway(project_dir)
    tools = []
